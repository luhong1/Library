...
$(function(){ 
	...
	$(".send").on("click", function(){
		...
		if(data.name == '' || data.name ==null  || data.name == "undefined"){
			$("#form-ipt-error-cl-name").addClass("is-visible");
            $("#form-ipt-error-cl-name").prev().addClass("input-error");
			$("#form-ipt-error-cl-name").html("请输入姓名");
			alert('请输入姓名');
			return;
		}else{
			$("#form-ipt-error-cl-name").removeClass("is-visible");
			$("#form-ipt-error-cl-name").prev().removeClass("input-error");
		}
		if(!reg2.test(data.name)){
			$("#form-ipt-error-cl-name").addClass("is-visible");
            $("#form-ipt-error-cl-name").prev().addClass("input-error");
			$("#form-ipt-error-cl-name").html("请输入合法的姓名");
			alert('请输入合法的姓名');
			return ;
		}else{
			$("#form-ipt-error-cl-name").removeClass("is-visible");
		...
		if(data.Id == '' || data.Id ==null  || data.Id == "undefined"){
			...
			$("#form-ipt-error-cl-code").html("请输入身份证号码");
			alert('请输入身份证号码');
			return;
		}else{
		...
		if(data.password == '' || data.password ==null  || data.password == "undefined"){
			...
			$("#form-ipt-error-cl-password").html("请输入以字母开头，长度在6-18之间的密码");
			alert("请输入密码");
			return ;
		}else{
		...
		if(!reg1.test(data.password)){
			$("#form-ipt-error-cl-password").addClass("is-visible");
            $("#form-ipt-error-cl-password").prev().addClass("input-error");
			$("#form-ipt-error-cl-password").html("请输入有效的密码");
			alert('请输入有效的密码');
			return ;
		}else{
			$("#form-ipt-error-cl-password").removeClass("is-visible");
		...
		$.ajax({
	        ...
	        success: function (data) {
	        	...
	        	if(data == 'IDfalse'){
	        		$("#form-ipt-error-cl-code").addClass("is-visible");
	                $("#form-ipt-error-cl-code").prev().addClass("input-error");
	    			$("#form-ipt-error-cl-code").html("请输入合法的身份证号码");
	        		alert("身份证号码错误");
	        		return ;
	        	}else{
	        		$("#form-ipt-error-cl-code").removeClass("is-visible");
	...
	$(".sub-b").on("click", function(){
		...
		if(data.mobile == '' || data.mobile ==null  || data.mobile == "undefined"){
			$("#form-ipt-error-cl-mobile").addClass("is-visible");
            $("#form-ipt-error-cl-mobile").prev().addClass("input-error");
			$("#form-ipt-error-cl-mobile").html("请输入手机号");
			alert("请输入手机号");
			return ;
		}else{
			$("#form-ipt-error-cl-mobile").removeClass("is-visible");
		...
		if(data.verifyCode == '' || data.verifyCode ==null  || data.verifyCode == "undefined"){
			$("#form-ipt-error-cl-verifyCode").addClass("is-visible");
            $("#form-ipt-error-cl-verifyCode").prev().addClass("input-error");
			$("#form-ipt-error-cl-verifyCode").html("请输入验证码");
			alert("请输入验证码");
			return ;
		}else{
		...
		$.ajax({
	        ...
	        success: function (data) {
	        	...
	        	if(data =='-1'){
	        		$("#form-ipt-error-cl-mobile").addClass("is-visible");
	                $("#form-ipt-error-cl-mobile").prev().addClass("input-error");
	    			$("#form-ipt-error-cl-mobile").html("手机号已注册");
	        		alert("手机号已注册");
	        		return ;
	        	}else{
	        		$("#form-ipt-error-cl-mobile").removeClass("is-visible");
	        	...
	        	if(data =='-2'){
	        		$("#form-ipt-error-cl-code").addClass("is-visible");
	                $("#form-ipt-error-cl-code").prev().addClass("input-error");
	    			$("#form-ipt-error-cl-code").html("身份证号已注册");
	        		alert("身份证号已注册");
	        		return ;
	        	}else{
	        		$("#form-ipt-error-cl-code").removeClass("is-visible");
      		...
      		<div class="container">
				...
				<div id="navbar" class="navbar-collapse collapse">
					<ul class="nav navbar-nav navbar-right">			            
							<li><a href="http://32m1v11333.eicp.vip/Library/login.jsp#studentID" ><i class="fa fa-user"></i> 登录</a></li>	
						<li><a href="http://32m1v11333.eicp.vip/Library/user_jsp/registerUser.jsp" ><i class="fa fa-user"></i> 注册</a></li>
					</ul>
...
<div class="row bg bg-success">
   ...
   <div class="col-sm-11">	
     <div class="row">
          	...
          	<div class="col-sm-5">
          	<input name="name"  autocomplete="off"  placeholder="请输入姓名" class="form-control">
           <span class="form-ipt-error errorico-em" id="form-ipt-error-cl-name">姓名不能为空</span>
     ...
     <div class="row">
      	<label for="firstname" class="col-sm-3 control-label" style="padding-top: 10px;">身份证号码:</label>
      		<div class="col-sm-5">
         <input name="id"  autocomplete="off"  placeholder="请输入身份证号码" class="form-control">
           <span class="form-ipt-error errorico-em" id="form-ipt-error-cl-code">身份证不能为空</span>
         <label class="control-label"  style="display: none"></label>
         </div>
       </div>
     <div class="row">
     	<label for="firstname" class="col-sm-3 control-label" style="padding-top: 10px;">密码:</label>
          <div class="col-sm-5">
          <input type="password"  autocomplete="off"  name="password" placeholder="以字母开头,长度不少于6" class="form-control">
            <span class="form-ipt-error errorico-em" id="form-ipt-error-cl-password">密码不能为空</span>
            <label class="control-label"  style="display: none"></label>
            </div>
       </div>
        <div class="row">
           ...
           <div class="col-sm-5">
          <input type="password"  autocomplete="off"  name="newpassword1" placeholder="请再次输入密码" class="form-control">
           <span class="form-ipt-error errorico-em" id="form-ipt-error-cl-password2">确认不能为空</span>
            <label class="control-label"  style="display: none"></label>
            </div>
       </div>
       <div class="row">
       ...
       <div class="row">
        <button type="button" class="sub-b btn btn-primary" style="width: 150px;margin-left: 15%">注册</button>
