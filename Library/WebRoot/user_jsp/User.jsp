			...
			<div class="col-xs-11.5 main">
				...
				<div class="panel panel-primary">
					...
					<div class="panel-heading">
						基本信息<i class="fa fa-thumb-tack pull-right"></i>
					</div>
					<div class="panel-body">
						<div class="row" style="margin-bottom: 15px">
							<table class="table table-bordered table-invoice">
								<tr>
									<td width="74px" class="taskStatus"><span class="done">账号</span></td>
									<td width="149px">&nbsp;<%=user.getLoginName() %></td>
								</tr>
								<tr>
									<td class="taskStatus"><span class="done">手机号</span></td>
									<td>&nbsp;<%=user.getPhoneNumber() %> <a href="#upPhonenumber" data-toggle="modal">修改</a></td>
									
								</tr>
								<tr>
									<td class="taskStatus"><span class="done">姓名</span></td>
									<td>&nbsp;<%=user.getTrueName() %></td>
								</tr>
								<tr>
									<td class="taskStatus"><span class="done">身份证号</span></td>
									<td>&nbsp;<%=user.getIdNumber() %></td>
								</tr>
								<tr>
									<td class="taskStatus"><span class="done">邮箱</span></td>
									<td>&nbsp;<%=user.getEmail() %></td>
								</tr>
								<tr>
									<td class="taskStatus"><span class="done">剩余借书数量</span></td>
									<td>&nbsp;<%=user.getMaxNum() %></td>
								</tr>
							</table>
						</div>
						<div class="panel-heading"
							style="background: #636363; width: 100%; height: 50px">
							<h5>借阅信息</h5>
						</div>
						<div class="row" style="margin-bottom: 15px">
							...
							<table class="table table-bordered" id="show_tab">
								...
								if (bookData == null) {
									...
									bookData = (ArrayList<History>) bookDao.getAllHistory3(user.getaId());
								}
								if(bookData!=null){
							...
							%>
								...
								<tbody>
									...
									<%if(h1.getStatus()==1) {%>
										<td><button type="button" class="btn btn-info btn-xs" data-toggle="modal" onclick="returnbook(<%=h1.gethId()%>)">还书</button></td>
									<%}else{%>
									<td><span class="btn btn-info btn-xs">已还</span></td>
									<%} %>
									</tr>
...
<form>
	 ...
	 <form>
		...
		<!-- 模态框（Modal） -->
		<div class="modal fade" id="upPhonenumber" tabindex="-1" role="dialog"
			aria-labelledby="ModalLabel" aria-hidden="true">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal"
							aria-hidden="true">&times;</button>
						<h4 class="modal-title" id="ModalLabel">修改手机号</h4>
					</div>
					...
					<div class="modal-body">
						...
						<input type="hidden" name="flag" value="2"> <input
							type="hidden" name="url" value="index">
						<div class="row">
							<label for="firstname" class="col-sm-3 control-label" style="padding-top: 10px;">原手机号</label>
							<div class="col-sm-7">
								<input type="text" class="form-control" id="phonenumber"
									name="phonenumber" readonly="readonly" value='<%=user.getPhoneNumber() %>'> 
									<label class="control-label"  style="display: none"></label>
							</div>
						</div>

						<div class="row">
							<label for="firstname" class="col-sm-3 control-label" style="padding-top: 10px;">新手机号</label>
							<div class="col-sm-4">
								<input type="text" class="form-control" id="phonenumber1" name="phonenumber1"
									placeholder="请输入新手机号">
									<span class="form-ipt-error errorico-em" id="form-ipt-error-cl-phonenumber1">新手机号不能为空</span>
									 <label class="control-label" style="display: none"></label></div>
									<div class="col-sm-3">
									<button type="button" class="userCode">获取验证码</button>
							</div>
						</div>
						<div class="row">
							...
							<div class="col-sm-7">
								<input type="text" class="form-control" id="code" name="code"
									placeholder="请输入验证码">
								<span class="form-ipt-error errorico-em" id="form-ipt-error-cl-code">验证码不能为空</span>
									 <label class="control-label" style="display: none"></label>
							</div>
						</div>
					</div>
					<div class="modal-footer">
						...
						<button type="button" class="sub-btn7 btn btn-primary">修改</button>
					</div>
				</div>
				<!-- /.modal-content -->
			</div>
	...
	<script type="text/javascript">
			function returnbook(hId) {
				con = confirm("是否还书?");
				if (con == true) {
					location.href = "http://120.26.187.198:8080/Library/borServlet?flag=3&room=1&hId=" + hId;
				}
			}
			function break1 () {
   ...
   <script type="text/javascript">
    	...
    	$(function(){ 
    		...
    		$(".sub-btn4").on("click", function(){
        		...
        		var $mobile=$("input[name=phonenumber1]");
        		var data = {};
        		data.code = $.trim($("input[name=code]").val());
        		data.mobile = $.trim($mobile.val());
        		if(data.mobile == '' || data.mobile ==null  || data.mobile == "undefined"){
        			$("#form-ipt-error-cl-phonenumber1").addClass("is-visible");
                    $("#form-ipt-error-cl-phonenumber1").prev().addClass("input-error");
        			$("#form-ipt-error-cl-phonenumber1").html("新手机号不能为空");
        			alert("请输入新手机号");
        			return;
        		}else{
        			$("#form-ipt-error-cl-phonenumber1").removeClass("is-visible");
        			$("#form-ipt-error-cl-phonenumber1").prev().removeClass("input-error");
        		}
        		...
        		if(data.code == '' || data.code ==null  || data.code == "undefined"){
        			...
        			$("#form-ipt-error-cl-code").html("请输入验证码");
        			alert("请输入验证码");
        			return ;
        		}else{
        		...
        		if(!reg.test(data.mobile)){
        			$("#form-ipt-error-cl-phonenumber1").addClass("is-visible");
                    $("#form-ipt-error-cl-phonenumber1").prev().addClass("input-error");
        			$("#form-ipt-error-cl-phonenumber1").html("请输入合法的手机号码");
        			alert('请输入合法的手机号码');
        			return false;
        		}else{
        			$("#form-ipt-error-cl-phonenumber1").removeClass("is-visible");
   ...
   $(function(){ 
		...
		var $mobile=$("input[name=phonenumber1]");
 		...
 		if(!reg.test(data.mobile)){
			...
			$.ajax({
		        url: "http://120.26.187.198:8080/Library/sendSms",
		        async : false,
		        type: "get",
		        dataType: "text",
		        data: data,
